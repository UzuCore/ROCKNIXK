...
then
  ...
  #Show FPS
	if [ "$FPS" = "false" ]
	then
  		sed -i '/^OsdShowFPS =/c\OsdShowFPS = false' /storage/.config/PCSX2/inis/PCSX2.ini
	fi
	if [ "$FPS" = "true" ]
	then
  		sed -i '/^OsdShowFPS =/c\OsdShowFPS = true' /storage/.config/PCSX2/inis/PCSX2.ini
	fi
...
#Run PCSX2 emulator
  ...
  ${EMUPERF} /usr/share/pcsx2-sa/pcsx2-qt -bigpicture -fullscreen "${1}"
